...
app.get("/api/patients", async (req, res) => {
    ...
    try {
        ...
        const args = [];
        if (q) {
            sql += ` WHERE CONCAT_WS(' ', nombre, apellido, dni, email, telefono) LIKE ?`;
            args.push(`%${q}%`);
        }
...
app.post("/api/patients", async (req, res) => {
    ...
    const conn = await pool.getConnection();
    try {
        const [r] = await conn.execute(`INSERT INTO patients (nombre, apellido, dni, email, telefono) VALUES (?,?,?,?,?)`, [nombre, apellido, dni || null, email || null, telefono || null]);
        const id = r.insertId;
...
app.put("/api/patients/:id", async (req, res) => {
    ...
    const conn = await pool.getConnection();
    try {
        await conn.execute(`UPDATE patients SET nombre=?, apellido=?, dni=?, email=?, telefono=? WHERE id=?`, [nombre, apellido, dni || null, email || null, telefono || null, req.params.id]);
        const [rows] = await conn.query(`SELECT * FROM patients WHERE id=?`, [req.params.id]);
        res.json(rows[0] || null);
    }
    finally {
...
app.get("/api/appointments", async (req, res) => {
    const { from, to, professional } = req.query;
    const conn = await pool.getConnection();
    try {
        ...
        if (from) {
            sql += ` AND a.date_utc >= ?`;
            args.push(new Date(String(from)));
        }
        if (to) {
            sql += ` AND a.date_utc <  ?`;
            args.push(new Date(String(to)));
        }
        if (professional) {
            sql += ` AND a.professional = ?`;
            args.push(String(professional));
        }
...
app.post("/api/appointments", async (req, res) => {
    const { patientId, professional, title = "Turno", reason = "", dateISO, durationMin = 30, status = "pending", location = "", isUrgent = 0, } = req.body || {};
    if (!patientId || !professional || !dateISO)
        return res.status(400).json({ error: "missing_fields" });
    const conn = await pool.getConnection();
    try {
        const dateUtc = parseToUtc(String(dateISO));
        const [r] = await conn.execute(`INSERT INTO appointments (patient_id, professional, title, reason, date_utc, duration_min, status, location, is_urgent)
       VALUES (?,?,?,?,?,?,?,?,?)`, [Number(patientId), professional, title, reason, dateUtc, Number(durationMin), status, location, isUrgent ? 1 : 0]);
        const id = r.insertId;
...
app.put("/api/appointments/:id", async (req, res) => {
    const { patientId, professional, title, reason, dateISO, durationMin, status, location, isUrgent, } = req.body || {};
    const conn = await pool.getConnection();
    try {
...
app.get("/api/payments", async (req, res) => {
    ...
    try {
        ...
        if (desde) {
            sql += ` AND pay.fecha >= ?`;
            args.push(String(desde));
        }
        if (hasta) {
            sql += ` AND pay.fecha <= ?`;
            args.push(String(hasta));
        }
        if (metodo) {
            sql += ` AND pay.metodo = ?`;
            args.push(String(metodo));
        }
        if (q) {
...
app.post("/api/payments", async (req, res) => {
    ...
    if (!pacienteId || !fecha || !metodo || !monto)
        return res.status(400).json({ error: "missing_fields" });
    const conn = await pool.getConnection();
    try {
        const [r] = await conn.execute(`INSERT INTO payments (paciente_id, fecha, metodo, concepto, monto, status)
       VALUES (?,?,?,?,?,?)`, [Number(pacienteId), fecha, metodo, concepto || "", Number(monto), status]);
        const id = r.insertId;
...
app.put("/api/payments/:id", async (req, res) => {
    ...
    const conn = await pool.getConnection();
    try {
        await conn.execute(`UPDATE payments SET paciente_id=?, fecha=?, metodo=?, concepto=?, monto=?, status=? WHERE id=?`, [Number(pacienteId), fecha, metodo, concepto || "", Number(monto), status, req.params.id]);
...
app.get("/api/dashboard/summary", async (_req, res) => {
    try {
        ...
        try {
            ...
            const [apptRows] = await conn.query(`SELECT status, COUNT(*) as c
         FROM appointments
         WHERE date_utc >= ? AND date_utc < ?
         GROUP BY status`, [startUtc, endUtc]);
            const [nextRows] = await conn.query(`SELECT a.id, a.title, a.professional, a.location, a.date_utc, a.duration_min,
                p.apellido, p.nombre
         FROM appointments a
         JOIN patients p ON p.id = a.patient_id
         WHERE a.date_utc >= ? AND a.date_utc < ?
         ORDER BY a.date_utc ASC
         LIMIT 1`, [startUtc, endUtc]);
            ...
            const [sumRows] = await conn.query(`SELECT COALESCE(SUM(monto),0) as total
         FROM payments
         WHERE fecha BETWEEN ? AND ? AND status='approved'`, [first, last]);
            const [newPacRows] = await conn.query(`SELECT COUNT(*) as total
         FROM patients
         WHERE created_at >= ? AND created_at < ?`, [mStartUtc, mEndUtc]);
            res.json({
                today: {
                    byStatus: Object.fromEntries(apptRows.map(r => [r.status, Number(r.c)])),
                    next: nextRows[0] ?? null,
                },
                month: {
                    revenueApproved: Number(sumRows[0]?.total ?? 0),
                    newPatients: Number(newPacRows[0]?.total ?? 0),
                }
            });
        }
        finally {
